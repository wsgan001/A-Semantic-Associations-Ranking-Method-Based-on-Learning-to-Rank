...
public class svm_scale
{
	...
	private static String savefilenm;
	private static BufferedWriter fl_save = null;
	private static DecimalFormat fm=new DecimalFormat("#.00000000");
	...
	private void output(int index, double value)
	{
		...
		//{
			...
			try {
				fl_save.write(index + ":" + fm.format(value) + " ");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public static void main(String argv[]) throws IOException
	{
		...
		Integer lenargv=argv.length;
		String[] tmp = new String[lenargv-1];
		for(Integer i=1;i<lenargv;++i)tmp[i-1]=argv[i];
		savefilenm = argv[0];
		try {
			fl_save = new BufferedWriter(new FileWriter(savefilenm));
		} catch(IOException e) {
		}
		s.run(tmp);
		fl_save.close();
